...
description: 为 openclaw-it-team 仓库生成和维护部署型工作流。用于把当前仓库的多 Agent 配置安装到 OpenClaw、本地化 `openclaw.json`、填充模型与飞书账号配置，并完成启动前验收。
---
...
- 不要假设用户机器是 `/root`；把 `docs/openclaw.json` 中的 `/root/.openclaw/...` 改成用户真实 Home 目录
- 不要在没拿到真实凭据前伪造“部署成功”
...
注意：仓库里的源目录是 `agents/pm` 这种结构，但配置里的目标目录是 `~/.openclaw/agents/pm/agent`。部署时应创建 `agent/` 这一层，并把对应文件复制进去。
...
```text
repo/agents/pm/* -> ~/.openclaw/agents/pm/agent/
repo/agents/rd/* -> ~/.openclaw/agents/rd/agent/
repo/agents/qa/* -> ~/.openclaw/agents/qa/agent/
repo/agents/ce/* -> ~/.openclaw/agents/ce/agent/
```
...
- `openclaw.json` 被加载，无 `agentDir` 缺失报错
